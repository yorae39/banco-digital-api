...
interface BankStatementRepository : JpaRepository<BankStatement, Long> {
    @Query(
        "SELECT new com.example.bancodigital.dto.BankStatementDTO(" +
                "t.description, t.value, bs.initialDate, bs.finalDate, bs.accountExternalKey, bs.transactionType) " +
                "FROM BankStatement bs JOIN bs.transactions t " +
                "WHERE bs.externalKey = :externalKey"
    )
    fun findSavedStatement(@Param("externalKey") externalKey: UUID): List<BankStatementDTO>
}
/*
interface BankStatementRepository : JpaRepository<BankStatement, Long> {
